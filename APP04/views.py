...
from  APP04.forms import *
from django.contrib.auth.hashers import make_password
import random
import json
import APP04.zhenzismsclient as smsclient
import hashlib
...
def Index(request):
    ...
    if type_id:
        ...
        news_list_all = Content.objects.all()
...
@login_required(login_url="/login")
def restpassword(request, user_id):
...
def send_dx(mobile):
    verific = "".join([str(random.randint(0, 9)) for i in range(4)])
    ...
    result = client.send(mobile, "您的验证码是%s" % verific)
    return json.loads(result), verific
        ...
        if form.is_valid():
            ...
            if user:
                ...
                if result.get("code") == 0:  # 结果的返回码为0,发送短信成功
                    return redirect('/getverification/%s/%s' % (user.id, hash_code(yzm)))
        return render(request, "forgetpwd.html", locals())


def getverification(request, user_id, code):
    if request.method == "GET":
        return render(request, "getverification.html", locals())
        ...
        if user and code == hash_code(new_code):
            login(request, user)
            return redirect("/restpassword/%s" % user_id)
        else:
            return render(request, "getverification.html", locals())
